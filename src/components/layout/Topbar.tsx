...
export default function Topbar() {
  ...
  // Export buttons are only shown on the Review step

  // layout save/load/delete removed

  return (
    <header className="w-full border-b border-border bg-card-2">
      <div className="max-w-[1200px] mx-auto flex items-center justify-between p-4">
        <div className="flex items-center gap-4 w-full">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            {step === 2 && (
              <div className="flex items-center gap-3">
                <button className="px-3 py-1 rounded bg-primary-500 text-white text-sm" onClick={onExportCsv}>Export CSV</button>
                <button className="px-3 py-1 rounded bg-primary-500 text-white text-sm" onClick={onExportPdf}>Export PDF</button>
              </div>
            )}
            {/* Theme controls moved to floating FAB */}
          </div>
        </div>
      </div>
    </header>
